...
#This file is used to load the weights and prepare the ground states
################################################################
...
target_data = [[0.0+0.000000000000000000e+00j,  0.0+0.000000000000000000e+00j],
...
op_down2 = tf.experimental.numpy.kron(op_down2,Iz)
def initial_prep():
    ini_theta = tf.random.uniform(shape=[2],minval=0,maxval=2*tnp.pi,dtype=DEFAULT_TENSOR_TYPE)
    state1 = inicuit(ini_theta)
    state1 = tf.reshape(state1,[1,2,2])
    state2 = inicuit(ini_theta)
    state2 = tf.reshape(state2,[1,2,2])
    state3 = inicuit(ini_theta)
    state3 = tf.reshape(state3,[1,2,2])
    state4 = inicuit(ini_theta)
    state4 = tf.reshape(state4,[1,2,2])
    state5 = inicuit(ini_theta)
...
def sample_prep():
    ...
    ip = ip/ ip_norm
    state = state1*ip[0] + state2*ip[1] + state3*ip[2] + state4*ip[3] + state5*ip[4]
    angle = tf.random.uniform(shape=[1,1],minval=0,maxval=2*tnp.pi,dtype=DEFAULT_TENSOR_TYPE)
    iupx = tf.math.cos(angle)
    iupz = iupy =  tf.math.sin(angle)/tf.cast(tf.math.sqrt(2.0),dtype=DEFAULT_TENSOR_TYPE)
...
class RNNModel(keras.Model):
    ...
    def train_step(self, data):
        ...
        self.mae_metric.update_state(dm5,target)
        return {"loss": self.loss_tracker.result(), "mae": self.mae_metric.result()}
    def predict_step(self, data):
        qs, target = data
        for i in range(100):
            initial_state, iupxyz = sample_prep()
            #1
            m_iupxyz0 = tf.reshape(iupxyz,[1,1,3])
            h_state11,c_state11 = self.model1(m_iupxyz0,None,None, training=True)
            dm1,measure1 = self.data_flowing(initial_state,h_state11,1)
            ...
            print(i)
            loss = sdloss(dm5,iupxyz)
            with open("pred.txt","a") as o:
                o.write(str(initial_state.numpy())+"\n"+str(dm1.numpy())+"\n"+str(dm2.numpy())+"\n"+str(dm3.numpy())+"\n"+str(dm4.numpy())+"\n"+str(dm5.numpy())+"\n")
            with open("loss.txt","a") as o:
                o.write(str(loss.numpy())+"\n")
        return dm5
